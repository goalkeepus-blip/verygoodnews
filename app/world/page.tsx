...
export default async function WorldPage() {
  const [{ data: articlesData }, { data: sidebarData }, { data: adData }] = await Promise.all([
    supabaseAdmin.from('articles').select('id, section, title, summary, thumbnail_url, published_at, view_count, author_name, video_url').eq('section', 'world').eq('status', 'published').eq('is_deleted', false).order('published_at', { ascending: false }).limit(24),
    supabaseAdmin.from('articles').select('id, title, thumbnail_url, published_at, video_url, summary, view_count, author_name, section').eq('section', 'world').eq('status', 'published').eq('is_deleted', false).order('published_at', { ascending: false }).limit(5),
    supabaseAdmin.from('ads').select('*').eq('slot_key', 'world_sidebar').single(),
  ])
    ...
    <main className="min-h-screen bg-forest-bg">
      <div className="max-w-[1200px] mx-auto px-4 py-8">
        <div className="mb-6 pb-3 border-b-2 border-forest-accent flex items-baseline gap-3">
          <span className="text-[11px] font-bold tracking-[0.2em] text-forest-accent uppercase">WORLD</span>
          <h1 className="font-serif text-2xl font-bold text-forest-dark">해외뉴스</h1>
          <span className="text-xs text-forest-muted ml-auto">세계의 따뜻한 소식을 전합니다</span>
        </div>
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_280px] gap-8">
          <div>
            {articles.length === 0 ? (
              <div className="flex items-center justify-center h-48 bg-white rounded-xl border border-forest-border text-sm text-forest-muted">아직 해외뉴스가 없습니다.</div>
            ) : (
              <MoreSection initialArticles={articles.slice(0, 12)} remaining={articles.slice(12)} section="world" />
            )}
          </div>
          <aside className="lg:sticky lg:top-24 self-start space-y-6">
            ...
            {sidebarArticles.length > 0 && (
              <div className="bg-white rounded-xl p-4 border border-forest-border shadow-sm">
                <div className="flex items-baseline gap-2 mb-3 pb-2 border-b border-forest-border">
                  ...
                  <span className="text-sm font-bold text-forest-dark">최신 해외뉴스</span>
                </div>
            ...
            <div className="bg-white rounded-xl p-4 border border-forest-border shadow-sm">
              ...
              <div className="space-y-2">
                {[{ label: '국내뉴스', href: '/domestic' }, { label: '영상뉴스', href: '/video' }, { label: '기획칼럼', href: '/column' }].map((item) => (
...
export const metadata = { title: '해외뉴스 | VeryGoodNews', description: '세계의 따뜻한 소식을 전합니다' }