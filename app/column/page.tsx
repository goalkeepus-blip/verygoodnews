...
function SidebarItem({ article }: { article: Article }) {
  return (
    <Link href={`/article/${article.id}`} className="group flex gap-3 py-2 border-b border-forest-border last:border-b-0">
      <div className="w-7 h-7 rounded-full bg-forest-dark flex items-center justify-center flex-shrink-0">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-[10px] text-forest-muted mt-0.5">{article.author_name} · {formatDate(article.published_at)}</p>
      </div>
    </Link>
  )
}
...
export default async function ColumnPage() {
  const [{ data: articlesData }, { data: sidebarData }, { data: adData }] = await Promise.all([
    supabaseAdmin.from('articles').select('id, section, title, summary, thumbnail_url, published_at, view_count, author_name, video_url').eq('section', 'column').eq('status', 'published').eq('is_deleted', false).order('published_at', { ascending: false }).limit(24),
    supabaseAdmin.from('articles').select('id, title, thumbnail_url, published_at, video_url, summary, view_count, author_name, section').eq('section', 'column').eq('status', 'published').eq('is_deleted', false).order('published_at', { ascending: false }).limit(5),
    supabaseAdmin.from('ads').select('*').eq('slot_key', 'column_sidebar').single(),
  ])
    ...
    <main className="min-h-screen bg-forest-bg">
      <div className="max-w-[1200px] mx-auto px-4 py-8">
        <div className="mb-6 pb-3 border-b-2 border-forest-accent flex items-baseline gap-3">
          <span className="text-[11px] font-bold tracking-[0.2em] text-forest-accent uppercase">COLUMN</span>
          <h1 className="font-serif text-2xl font-bold text-forest-dark">기획·칼럼</h1>
          <span className="text-xs text-forest-muted ml-auto">깊이 있는 시선을 전합니다</span>
        </div>
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_280px] gap-8">
          <div>
            {articles.length === 0 ? (
              <div className="flex items-center justify-center h-48 bg-white rounded-xl border border-forest-border text-sm text-forest-muted">아직 칼럼이 없습니다.</div>
            ) : (
              <MoreSection initialArticles={articles.slice(0, 12)} remaining={articles.slice(12)} section="column" />
            )}
          </div>
          <aside className="lg:sticky lg:top-24 self-start space-y-6">
            ...
            {sidebarArticles.length > 0 && (
              <div className="bg-white rounded-xl p-4 border border-forest-border shadow-sm">
                <div className="flex items-baseline gap-2 mb-3 pb-2 border-b border-forest-border">
                  ...
                  <span className="text-sm font-bold text-forest-dark">최신 칼럼</span>
                </div>
            ...
            <div className="bg-white rounded-xl p-4 border border-forest-border shadow-sm">
              ...
              <div className="space-y-2">
                {[{ label: '국내뉴스', href: '/domestic' }, { label: '해외뉴스', href: '/world' }, { label: '영상뉴스', href: '/video' }].map((item) => (
...
export const metadata = { title: '기획·칼럼 | VeryGoodNews', description: '깊이 있는 시선을 전합니다' }